...
async function validateMigration() {
  try { const payload = await request(`/api/islice-instances/${$("migrationInstanceId").value}/migration/validate`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(migrationValues()) }); $("migrationResult").textContent = payload.ready ? "新节点 API 可用，可以接管。已完成窗口继续使用归档地址，未完成窗口后续将生成新的 task ID。" : `新节点不可用：${payload.newNode?.message || "连接失败"}`; $("migrationResult").hidden = false; $("migrationDialog").dataset.ready = payload.ready ? "1" : ""; } catch (error) { $("migrationResult").textContent = error.message; $("migrationResult").hidden = false; }
}
