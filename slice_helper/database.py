...
class Database:
    ...
    async def initialize(self) -> None:
        ...
        async with self.connect() as db:
            ...
            await db.executescript(
                ...
                CREATE TABLE IF NOT EXISTS resplit_batches (
                    id TEXT PRIMARY KEY,
                    job_id TEXT NOT NULL REFERENCES jobs(id) ON DELETE CASCADE,
                    start_window_index INTEGER NOT NULL,
                    end_window_index INTEGER NOT NULL,
                    status TEXT NOT NULL,
                ...
                CREATE INDEX IF NOT EXISTS idx_resplit_batches_job_created
                    ON resplit_batches(job_id, created_at DESC);

                CREATE TABLE IF NOT EXISTS resplit_batch_windows (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    batch_id TEXT NOT NULL REFERENCES resplit_batches(id) ON DELETE CASCADE,
                    window_id INTEGER NOT NULL REFERENCES windows(id) ON DELETE CASCADE,
                    window_index INTEGER NOT NULL,
                    original_task_id TEXT NOT NULL,
                    candidate_task_id TEXT NOT NULL UNIQUE,
                    ...
                    finished_at TEXT,
                    updated_at TEXT NOT NULL,
                    UNIQUE(batch_id, window_index)
                );
                CREATE INDEX IF NOT EXISTS idx_resplit_batch_windows_batch
                    ON resplit_batch_windows(batch_id, window_index);
                """
            )
            await db.execute(
    ...
    async def create_resplit_batch(
        self,
        batch_id: str,
        job_id: str,
        ...
        async with self.connect() as db:
            ...
            active = await (
                await db.execute(
                    "SELECT id FROM resplit_batches WHERE job_id=? "
                    "AND status IN ('queued', 'running', 'committing')",
                    (job_id,),
                )
            ).fetchone()
            if active is not None:
                await db.rollback()
                raise ValueError("This job already has a range resplit in progress")
            await db.execute(
                """
                INSERT INTO resplit_batches(
                    id, job_id, start_window_index, end_window_index, status,
                    total_window_count, affected_segment_count, created_at, updated_at
                ) VALUES(?, ?, ?, ?, 'queued', ?, ?, ?, ?)
                """,
                (
                    batch_id,
            ...
            await db.executemany(
                """
                INSERT INTO resplit_batch_windows(
                    batch_id, window_id, window_index, original_task_id,
                    candidate_task_id, nominal_end, status, updated_at
                ) VALUES(?, ?, ?, ?, ?, ?, 'queued', ?)
                """,
                [
                    (
                        ...
                        str(item["original_task_id"]),
                        str(item["candidate_task_id"]),
                        float(item["nominal_end"]),
                        now,
                    )
                    for item in windows
                ],
            ...
            row = await (
                await db.execute("SELECT * FROM resplit_batches WHERE id=?", (batch_id,))
            ).fetchone()
            await db.commit()
        if row is None:
            raise RuntimeError("Failed to create range resplit batch")
        return dict(row)

    async def get_resplit_batch(self, batch_id: str) -> dict[str, Any] | None:
        async with self.connect() as db:
            row = await (
                await db.execute("SELECT * FROM resplit_batches WHERE id=?", (batch_id,))
            ).fetchone()
    ...
    async def get_latest_resplit_batch(self, job_id: str) -> dict[str, Any] | None:
        async with self.connect() as db:
            row = await (
                await db.execute(
                    "SELECT * FROM resplit_batches WHERE job_id=? "
    ...
    async def get_resplit_batch_windows(self, batch_id: str) -> list[dict[str, Any]]:
        async with self.connect() as db:
            rows = await (
                await db.execute(
                    "SELECT * FROM resplit_batch_windows WHERE batch_id=? ORDER BY window_index",
                    (batch_id,),
                )
            ).fetchall()
        return [dict(row) for row in rows]
    ...
    async def update_resplit_batch(self, batch_id: str, **fields: Any) -> None:
        ...
        if unknown:
            raise ValueError(f"Unknown resplit batch fields: {sorted(unknown)}")
        if not fields:
            return
        fields["updated_at"] = utc_now()
        assignments = ", ".join(f"{key}=?" for key in fields)
        async with self.connect() as db:
            await db.execute(
                f"UPDATE resplit_batches SET {assignments} WHERE id=?",
                (*fields.values(), batch_id),
            )
            await db.commit()

    async def update_resplit_batch_window(self, row_id: int, **fields: Any) -> None:
        unknown = set(fields) - self.RESPLIT_BATCH_WINDOW_FIELDS
        if unknown:
            raise ValueError(f"Unknown resplit batch window fields: {sorted(unknown)}")
        if not fields:
            return
        fields["updated_at"] = utc_now()
        assignments = ", ".join(f"{key}=?" for key in fields)
        async with self.connect() as db:
            await db.execute(
                f"UPDATE resplit_batch_windows SET {assignments} WHERE id=?",
    ...
    async def recover_interrupted_range_resplits(self) -> list[dict[str, Any]]:
        ...
        async with self.connect() as db:
            ...
            await db.execute(
                "UPDATE resplit_batch_windows SET status='failed', error_message=?, "
                "finished_at=?, updated_at=? WHERE batch_id IN "
                "(SELECT id FROM resplit_batches WHERE error_message=?) "
                "AND status IN ('queued', 'cutting', 'submitting', 'polling', 'candidate_ready')",
    ...
    async def commit_resplit_batch(
        ...
        *,
        ...
        segment_columns = (
            ...
            "absolute_end", "title", "content_type", "news_event_type", "topic",
            "keywords_json", "summary", "segment_url", "cover_img_url", "attempt_id",
            "task_id", "raw_json",
        )
        async with self.connect() as db:
            await db.execute("BEGIN IMMEDIATE")
            ...
            if batch is None or batch["status"] not in {"running", "committing"}:
                await db.rollback()
                raise ValueError("Range resplit batch is not ready to commit")
            ...
            for candidate in adopted_rows:
                ...
                attempt_no_row = await (
                    await db.execute(
                        "SELECT COALESCE(MAX(attempt_no), 0) + 1 AS value FROM attempts WHERE window_id=?",
                        (candidate["window_id"],),
                    )
                ).fetchone()
                attempt_no = int(attempt_no_row["value"])
                attempt = await (
                    await db.execute(
                        """
                        INSERT INTO attempts(
                            window_id, attempt_no, task_id, status, service_status, progress,
                            raw_response_path, error_message, created_at, submitted_at, finished_at
                        ) VALUES(?, ?, ?, 'completed', 'completed', 100, ?, '', ?, ?, ?) RETURNING id
                        """,
                        (
                            candidate["window_id"],
                            attempt_no,
                            ...
                            now,
                            candidate["submitted_at"] or now,
                            candidate["finished_at"] or now,
                        ),
                    )
                ).fetchone()
                await db.execute("DELETE FROM segments WHERE window_id=?", (candidate["window_id"],))
                    ...
                    values = {
                        **segment,
                        ...
                        "absolute_start": absolute_time(
                            job["program_start_time"], float(segment["global_start"])
                        ),
                        "absolute_end": absolute_time(
                            job["program_start_time"], float(segment["global_end"])
                        ),
                        "attempt_id": attempt["id"],
                        "task_id": candidate["candidate_task_id"],
                    }
                    await db.execute(
                        f"INSERT INTO segments ({', '.join(segment_columns)}) "
                        f"VALUES ({', '.join('?' for _ in segment_columns)})",
                        tuple(values[column] for column in segment_columns),
                    )
                await db.execute(
                    "UPDATE windows SET requested_start=?, handoff_start=?, status='completed', "
                    "error_message='', updated_at=? WHERE id=?",
                    (
                        candidate["requested_start"],
                        candidate["handoff_start"],
                        now,
                        candidate["window_id"],
                    ),
                )
